...
CREATE INDEX idx_research_projects_owner ON research_projects(owner_user_id, created_at DESC);
...
CREATE UNIQUE INDEX uq_source_documents_doi ON source_documents(doi) WHERE doi IS NOT NULL;
CREATE UNIQUE INDEX uq_source_documents_patent_number ON source_documents(patent_number) WHERE patent_number IS NOT NULL;
CREATE UNIQUE INDEX uq_source_documents_content_hash ON source_documents(content_hash) WHERE content_hash IS NOT NULL;
CREATE INDEX idx_source_documents_type ON source_documents(source_type, created_at DESC);
CREATE INDEX idx_source_documents_title ON source_documents (lower(title) text_pattern_ops);
...
CREATE INDEX idx_search_results_query ON search_results(search_query_id, rank);
...
CREATE INDEX idx_ai_summaries_document ON ai_summaries(source_document_id, created_at DESC);
...
CREATE TRIGGER trg_comparisons_updated_at BEFORE UPDATE ON comparisons
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();
